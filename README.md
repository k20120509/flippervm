...
> - `momentum-fw-v1.1.5.dfu`(848 KB)— 真实的 Momentum v1.1.5 中文版固件,加载后点「运行」约 5M 条指令即可进入主菜单,LCD 显示 Flipper 启动画面。
...
| ✅ SysTick 滴答定时器 | 完整支持(含 COUNTFLAG) |
| ✅ USART1 / USART2 调试串口 | 完整支持,控制台可见 |
| ✅ NVIC 中断控制器 | 基础支持(抢占关闭) |
| ✅ RCC 时钟 / FLASH / PWR 电源 / GPIO | 完整支持,关键就绪标志位生效 |
| ✅ SPI2(显示控制器) | 完整支持 |
| ✅ FreeRTOS(SVC / PendSV / 上下文切换) | 完整支持 |
| ✅ IPCC 双核通信 / HWSEM 硬件信号量 | 仿真就绪,防止 CPU2 等待死循环 |
| ✅ RNG 随机数 / PKA 密码加速器 | 仿真就绪 |
| ✅ RTC 实时时钟 | 仿真就绪 |
| ✅ DWT 周期计数器(CYCCNT) | 完整支持,延时循环可正常退出 |
| 🔧 USB / 蓝牙射频 | 未仿真 |
...
> 上面「未仿真」的外设在读取时会返回 0 或触发未映射内存处理。真实固件目前已能**成功加载、解析向量表、执行 10 万+ 条 Thumb-2 指令并正常推进启动流程**(包括 BSS 清零、HAL 初始化、FreeRTOS 调度器启动等阶段),但因为缺少 CC1101/NFC/红外/USB/SD 等驱动,固件在初始化这些外设时会卡住,大概率跑不到菜单界面。本软件目前的主要用途是:**调试 / 逆向 / 研究固件启动流程**。想要完整功能?欢迎在 GitHub 提 Issue 或 PR 补齐外设,每次加一个外设固件就能往前走一步。
